...
import { adminNavItems } from "./layout-data";
    ...
    <div className="min-h-screen flex bg-admin-bg">
      ...
      <aside className="hidden lg:flex lg:flex-col lg:w-64 lg:fixed lg:inset-y-0 bg-sidebar border-r border-sidebar-border">
        ...
        <div className="flex items-center gap-2 px-4 h-16 border-b border-sidebar-border">
          <div className="flex items-center justify-center w-9 h-9 rounded-lg bg-admin-accent/20">
            <svg className="h-5 w-5 text-admin-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285Z" />
            </svg>
          </div>
          <div>
            <span className="font-bold text-sidebar-foreground">ResQ</span>
            <span className="text-xs text-admin-accent block font-medium">Admin Portal</span>
          </div>
        </div>
        ...
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto" aria-label="Admin navigation">
          {adminNavItems.map((item) => (
            ...
            >
              <span aria-hidden="true">{ADMIN_NAV_ICONS[item.to] ?? null}</span>
              {item.label}
            </NavLink>
          ))}
        </nav>
        ...
        <div className="p-4 border-t border-sidebar-border">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-10 h-10 rounded-full bg-admin-accent/10 flex items-center justify-center">
              <span className="text-sm font-semibold text-admin-accent">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-admin-accent truncate">Administrator</p>
            </div>
          </div>
          <Button
            variant="ghost"
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-panel-950 border-b border-white/10">
        <div className="flex items-center justify-between px-4 h-16">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <span className="text-xs text-admin-accent block">Admin</span>
            </div>
          </div>
        ...
        {/* Mobile Nav */}
        {sidebarOpen && (
          <nav className="px-4 py-3 bg-panel-950 border-b border-white/10 space-y-1 max-h-[60vh] overflow-y-auto">
            {adminNavItems.map((item) => (
              <NavLink
                key={item.to}
                to={item.to}
